...
\begin{proposition}[Evolution Strategy on a Flat Landscape]
    ...
    \begin{proposition}[Evolution Strategy on a Linear Landscape]
    ...
    The mean of the ES update is purely aligned with the gradient direction:
    \[
        ...
        \mathbb{E}[P^{\perp}\Delta\theta] = 0.
    \]
    The on- and off-manifold covariances are
    \[
        \operatorname{Cov}[P^{\parallel}\Delta\theta]
        = \frac{\alpha^2(2\sigma^2\|\mathbf{v}\|^2 + \sigma_\xi^2)}{N(\sigma^2\|\mathbf{v}\|^2 + \sigma_\xi^2)}\,P^{\parallel},
        \qquad
        \operatorname{Cov}[P^{\perp}\Delta\theta]
        = \frac{\alpha^2}{N}\,P^{\perp}.
    \]
    The off-manifold covariance is isotropic and independent of the signal,
    whereas the on-manifold covariance is modulated by the SNR: in the
    noiseless limit $\sigma_\xi \to 0$ it approaches $\frac{2\alpha^2}{N}P^{\parallel}$,
    and in the noise-dominated limit $\sigma_\xi^2 \gg \sigma^2\|\mathbf{v}\|^2$
    it recovers the flat-landscape value $\frac{\alpha^2}{N}P^{\parallel}$.
    ...
    \begin{proposition}[Evolution Strategy on a Linear Landscape]
        Let $R(\theta) = -\mathbf{v}^\top\theta$ with observation noise
        $\sigma_\xi$, and define $\sigma_R^2 := \sigma^2\|\mathbf{v}\|^2 + \sigma_\xi^2$.
        Since $\Delta\theta = P^{\parallel}\Delta\theta + P^{\perp}\Delta\theta$
        and the two components are independent, the mean and covariance of the
        full ES update follow by direct combination:
        \[
            \mathbb{E}[\Delta\theta]
            = \frac{-\sigma\alpha}{\sigma_R}\,\mathbf{v},
        \]
        \[
            \operatorname{Cov}[\Delta\theta]
            = \frac{\alpha^2}{N}\,I_d
            \;+\;
            \frac{\alpha^2\sigma^2}{N\sigma_R^2}\,\mathbf{v}\mathbf{v}^\top.
        \]
        The covariance decomposes into two interpretable parts: (i) an
        \emph{isotropic diffusion} term $\frac{\alpha^2}{N}I_d$, identical to
        the flat-landscape result and present regardless of any signal; and
        (ii) a \emph{rank-1 signal-aligned boost} $\frac{\alpha^2\sigma^2}{N\sigma_R^2}\mathbf{v}\mathbf{v}^\top$,
        which inflates variance specifically along the gradient direction $\mathbf{v}$
        in proportion to the SNR $s = \sigma^2\|\mathbf{v}\|^2/\sigma_R^2$.
        Consequently, the gradient direction is the \emph{noisiest} direction
        in parameter space, with variance $\frac{\alpha^2}{N}(1+s)$ along
        $\mathbf{v}/\|\mathbf{v}\|$, versus $\frac{\alpha^2}{N}$ in all orthogonal
        directions. The on-manifold fraction of total squared displacement is
        \[
            \rho :=
            \frac{\mathbb{E}\|P^{\parallel}\Delta\theta\|^2}
                 {\mathbb{E}\|\Delta\theta\|^2}
            ...
            \frac{1 + (N+1)\,s}
                 {d + (N+1)\,s},
        \]
        which increases monotonically in $s$ and $N$, and decreases in $d$.
        Since $N \ll d$ in practice, on-manifold movement per step is negligible
        relative to off-manifold diffusion.
        \end{proposition}
    ...
    \begin{proposition}[Evolution Strategy on a Quadratic Landscape]
    ...
    \begin{equation}
        \sigma_R^2 := \sigma^2\|\mathbf{v}\|^2
                    ...
                    + \sigma_\xi^2
        \label{eq:es-quadratic-sigmaR}
    \end{equation}
    The mean and covariance of the ES parameter update are
    \[
        \mathbb{E}[\Delta\theta]
        = \frac{-\alpha\sigma\,\mathbf{v}}{\sigma_R},
    \]
    \[
        \operatorname{Cov}[\Delta\theta]
        = \frac{\alpha^2}{N\sigma_R^2}
          ...
          \Bigr).
    \]
    Relative to the linear case, the quadratic term $Q$ introduces two
    ...
    $2\sigma^4 Q^2/\sigma_R^2$ in the covariance, which directs
    stochastic fluctuations along the principal curvature directions
    of $Q$.
\end{proposition}

\begin{proposition}[Dynamics of ES on Quadratic Landscape]
...
\subsubsection{Quadratic landscape analysis}
...
Note the order of the terms, using 1st term as reference,

the 2nd term is of the order $\frac{\sigma^{2}}{\sigma_{R}^{2}}\|\mathbf{v}\|^{2}$
on the gradient direction 

the 3rd term is of the order $\frac{2\sigma^{4}}{\sigma_{R}^{2}}\mathbf{u}^{\top}Q^{2}\mathbf{u}$
along each direction $\mathbf{u}$. 

Since in practise, $2\sigma^{2}$ is very small $4.5\times10^{-6}$,
generally the quadratic term is small and is small perturbation. Compare
the 2nd and 3rd term along a specific direction $\mathbf{u}$, we'd
derive the condition when the 3rd quadratic term has comparable effect.
\begin{align*}
...
\subsubsection{Multi-step analysis for quadratic landscape}
...
\begin{itemize}
\item In the extreme case, $\lambda_{k}=0$, then $\mathrm{Cov}[\mathbf{u}_{k}^{\top}\theta_{t}]=\frac{\alpha^{2}}{N}t$,
recovering the linear scaling of random walk. Similarly for $\lambda_{k}=\frac{2\sigma_{R}}{\alpha\sigma}$,
it increase linearly. 
\item $\lambda_{k}<0$ i.e. concave directions, $(1-\frac{\alpha\sigma}{\sigma_{R}}\lambda_{k})^{2}>1$,
...
\end{itemize}
\end{itemize}
In summary, for the simplified process 
